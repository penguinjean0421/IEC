...
public class Grappling : MonoBehaviour
{
    ...
    private void ExecuteGrapple()
    {
        ...
        float dynamicOvershoot = overshootYAxis + (distance * 0.3f); 

        float grapplePointRelativeYPos = Vector3.Dot(grapplePoint - lowestPoint, gravityUp);
        float highestPointOnArc = grapplePointRelativeYPos + dynamicOvershoot;

        if (grapplePointRelativeYPos < 0) highestPointOnArc = dynamicOvershoot;

        Vector3 finalDestination = grapplePoint + (gravityUp * 1.5f); 
    ...
    public void StopGrapple()
    {
        ...
        // 🌟 그래플이 끝나고 공중에서 떨어질 때, 천천히 떨어지도록 감속 플래그 ON!
        pm.isDroppingFromGrapple = true; 
    }
